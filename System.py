...
def view_employee_list():
    ...
    for emp in system_records:
      if len(emp) >= 5 and emp[4].strip().capitalize()==target_status:
...
def calculate_payroll():
    ...
    with open('payroll_history.csv', mode='a', newline='') as file:
        ...
        for emp in system_records:
            ...
            emp_id,f_name,l_name,role,status= emp[0],emp[1],emp[2],emp[3],emp[4]
            ...
            if status.strip().capitalize()== "Active":
                try:
                    ...
                    rate=float(input(f"Enter hourly rate for {f_name}:  "))
                    ...
                    writer.writerow([run_date,emp_id,f_name,l_name,role,hrs,rate,gross,nis,ed_tax,net])
